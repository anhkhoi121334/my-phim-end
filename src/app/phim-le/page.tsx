  ...
  useEffect(() => {
    const fetchSingleMovies = async () => {
      try {
        ...
        const response = await movieApi.fetchSingleMovies(currentPage);
        ...
        setError('Đã xảy ra lỗi khi tải phim lẻ. Vui lòng thử lại sau.');
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="container mx-auto p-4 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg">Đang tải danh sách phim lẻ...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <main className="container mx-auto px-4 py-8 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Phim Lẻ</h1>
          <p className="text-gray-400 mt-1">Tuyển tập phim lẻ hay nhất</p>
        </div>
      ...
      {movies.length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-xl text-gray-400">Không tìm thấy phim lẻ nào</p>
        </div>
      )}
    </main>
  );
