  ...
  useEffect(() => {
    const fetchTheaterMovies = async () => {
      try {
        ...
        const response = await movieApi.fetchTheaterMovies(currentPage);
        ...
        console.error('Lỗi khi tải phim chiếu rạp:', err);
        setError('Đã xảy ra lỗi khi tải phim chiếu rạp. Vui lòng thử lại sau.');
        setIsLoading(false);
      }
    };
    ...
    return (
      <div className="container mx-auto p-4 min-h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg">Đang tải danh sách phim chiếu rạp...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <main className="container mx-auto px-4 py-8 min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-white">Phim Chiếu Rạp</h1>
          <p className="text-gray-400 mt-1">Tuyển tập phim chiếu rạp mới nhất, chất lượng cao</p>
        </div>
      ...
      {movies.length > 0 ? (
        ...
        <div className="text-center py-12">
          ...
          <p className="text-xl text-gray-400">Không tìm thấy phim chiếu rạp nào</p>
        </div>
      )}
    </main>
  );
